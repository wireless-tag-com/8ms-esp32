...
bool touchpad_gslx680n_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    uint16_t tp_x,tp_y;
    uint8_t press;
    gslx680n_read_status(&tp_x,&tp_y,&press);
    data->state = press;
        switch (g_tp_roll)
        {
        case SCREEN_ORIGIN:
...
void qmsd_cus_board_init(qmsd_screen_rotation_t dir)
{
    ESP_LOGI(TAG,"Init board: for gslx680n");
    ...
    if (buf1) {
        lvgl_display_init(320, 320, buf1, NULL, qmsd_buf_size);
    }
    gslx680n_init(15,14,13,12);
    static lv_indev_drv_t indev_drv;               /*Descriptor of an input device driver*/
    lv_indev_drv_init(&indev_drv);          /*Basic initialization*/

    indev_drv.type = LV_INDEV_TYPE_POINTER; /*The touchpad is pointer type device*/
    indev_drv.read_cb = touchpad_gslx680n_read;            /*Library ready your touchpad via this function*/
