...
* @param mode 引脚模式，有输出两种，输入三种，详见 @ref gpio_pinmode_t
*/
...
void pinMode(gpio_num_t gpio_num, gpio_pinmode_t mode)
{
    ...
    if (gpioQueueSend != 0)
    {
        ESP_LOGI(TAG, "pinMode gpioQueueSend is ok!\n");
...
char digitalRead(gpio_num_t gpio_num)
{
    ...
    {
        if(xQueueReceive(gpioQueueReceive, (void *) &receive, (TickType_t)10))
        {
...
* @brief 设置写入电平的消息
* @param gpio_num 引脚编号，注意有些引脚功能不完全，详见 @ref gpio_num_t 
* @param level 引脚电平的高低，LOW或者HIGH，详见 @ref gpio_action_t
*/
...
void pinLoop(void)
{
    ...
    if (gpioQueueSend != 0)
    {
        ...
        {
            ESP_LOGI(TAG, "pinLoop gpioQueueSend is ok!\n");
            switch (receive->ID)
            {
            case PINMODE:
                ESP_LOGI(TAG, "PIN_MODE_OUTPUT!\n");
                __pinMode(receive->gpio_num, receive->mode);
                break;
            case DIGITALREAD:
                ESP_LOGI(TAG, "DIGITALREAD!\n");
                pinLevel = __digitalRead(receive->gpio_num);
                device_gpio* recv = (device_gpio*)gc_malloc(&gc,sizeof(device_gpio));
                recv->level = pinLevel;
                recv->ID = DIGITALREAD;
                if (gpioQueueReceive != 0)
                {
                    ESP_LOGI(TAG, "pinLoop gpioQueueReceive is ok!\n");
                    xQueueSend(gpioQueueReceive, (void *) &recv, (TickType_t)0);
                }
                break;
            case DIGITALWRITE:
                ESP_LOGI(TAG, "DIGITALWRITE!\n");
                __digitalWrite(receive->gpio_num, receive->level);
                break;
            default:
